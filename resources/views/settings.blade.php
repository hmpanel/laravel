    ...
    <div class="col-xl-6">
        <div class="card mb-4">
            <div class="card-header">
                ...
                {{ __('hmpanel.change_username') }}
            </div>
            <div class="card-body">
                <p>{{ __('hmpanel.current_username') }}: <b><span id="currentuser"></span></b></p>
                <div class="input-group">
                    <input class="form-control" type="text" placeholder="{{ __('hmpanel.change_username_placeholder') }}" id="newuser" autocomplete="off" />
    ...
    <div class="col-xl-6">
        <div class="card mb-4">
            <div class="card-header">
                ...
                {{ __('hmpanel.change_password') }}
            </div>
            <div class="card-body">
                <p>{{ __('hmpanel.update_password') }}</p>
                <div class="input-group">
                    <input class="form-control" type="text" placeholder="{{ __('hmpanel.change_password_placeholder') }}" id="newpass" autocomplete="off" />
                    <div class="input-group-append">
    ...
    <div class="col-xl-6">
        <div class="card mb-4">
            <div class="card-header">
                ...
                {{ __('hmpanel.panel_url') }}
            </div>
            <div class="card-body">
                <p>{{ __('hmpanel.panel_url_text') }}</p>
                <div class="input-group">
                    <input class="form-control" type="text" placeholder="panel.domain.ltd" id="panelurl" autocomplete="off" />
                    <div class="input-group-append">
                        <button class="btn btn-warning" type="button" id="panelurlssl" data-toggle="tooltip" data-placement="top" title="{{ __('hmpanel.panel_url_force_ssl') }}"><i class="fas fa-lock"></i></button>
                    </div>
                    <div class="input-group-append">
    ...
    <div class="col-xl-6">
        <div class="card mb-4">
            <div class="card-header">
                ...
                {{ __('hmpanel.panel_api') }}
            </div>
            <div class="card-body">
                <p>{{ __('hmpanel.panel_api_endpoint') }}:  <b>{{ URL::to('/api/') }}</b></p>
                <div class="text-center">
                    <button class="btn btn-primary mr-3" type="button" id="newapikey">{{ __('hmpanel.renew_api_key') }}</button>
                    <a href="/api/docs" target="_blank">
                        <button class="btn btn-warning" type="button">{{ __('hmpanel.documentation') }}</button>
                    </a>
                </div>
                <div class="space"></div>
            </div>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="authorizeModalLabel">{{ __('hmpanel.action_authorization_modal_title') }}</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <p>{{ __('hmpanel.action_authorization_modal_text') }}:</p>
                <div class="input-group">
                    <input class="form-control" type="password" id="currentpass" />
                </div>
                <div class="space"></div>
                <div class="text-center">
                    <button class="btn btn-primary" type="button" id="submit">{{ __('hmpanel.submit') }} <i class="fas fa-circle-notch fa-spin d-none" id="loading"></i></button>
                </div>
                <div class="space"></div>
            </div>
        </div>
        ...
        if(!oldpassval || oldpassval.length < 8) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if(patchcall == 'changeuser') {
                        success('{{ __('hmpanel.username_updated_success') }}');
                        $('#currentuser').html(localStorage.username);
                        $('#username').html(localStorage.username);
                    }
                    if(patchcall == 'changepass') {
                        success('{{ __('hmpanel.password_updated_success') }}');
                    }
                    if(patchcall == 'newapikey') {
                        success('{{ __('hmpanel.new_api_key_success') }}:<br><b>'+data.apikey+'</b>');
                    }
